...
function PANEL:Init()
    ...
    hook.Add("RenderScreenspaceEffects", "liaCharMenuDarken", function()
        ...
        local inHalf = lia.gui.character.inMainMenu
        if not inFull and not inHalf then return end
        local brightness = inFull and -0.3 or -0.15
        local contrast = inFull and 0.6 or 0.8
        local colour = inFull and 0.6 or 0.8
        DrawColorModify({
            ["$pp_colour_addr"] = 0,
            ["$pp_colour_addg"] = 0,
            ["$pp_colour_addb"] = 0,
...
function PANEL:loadBackground()
    if IsValid(self.themeTint) then
        self.themeTint:Remove()
        self.themeTint = nil
    end
