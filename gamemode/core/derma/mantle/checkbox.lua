...
function PANEL:Init()
    ...
    self._circle = 0
    self._circleEased = 0
    self._circleColor = table.Copy(lia.color.gray)
...
function PANEL:Paint(w, h)
    RNDX().Rect(0, 0, w, h):Rad(12):Color(lia.color.window_shadow):Shape(RNDX.SHAPE_IOS):Shadow(6, 22):Draw()
    RNDX().Rect(0, 0, w, h):Rad(12):Color(lia.color.focus_panel):Shape(RNDX.SHAPE_IOS):Draw()
    local textX = 14
    draw.SimpleText(self.text, 'Fated.18', textX, h * 0.5, lia.color.text, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
end
...
function PANEL:PaintOver(w, h)
    ...
    RNDX().Rect(trackX, trackY + 1, trackW, trackH - 2):Rad(trackH / 2):Color(lia.color.toggle):Shape(RNDX.SHAPE_IOS):Draw()
    local circleSize = 20
    local pad = 0
    local textMargin = 14
    ...
    local baseCircle = self.value and currentTheme.theme or lia.color.gray
    local circleCol = table.Copy(baseCircle)
    circleCol.a = 255
    self._circleColor = lia.color.Lerp(12, self._circleColor, circleCol)
    RNDX().Circle(circleCenterX, circleCenterY, circleSize):Color(self._circleColor):Draw()
    RNDX().Circle(circleCenterX, circleCenterY + 2, circleSize * 1.05):Color(Color(0, 0, 0, 30)):Draw()
end
