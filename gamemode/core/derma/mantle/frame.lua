...
function PANEL:Init()
    ...
    self.cls:SetText('')
    self.cls.Paint = function(_, w, h) RNDX().Rect(2, 2, w - 4, h - 4):Color(lia.color.header_text):Material(mat_close):Draw() end
...
function PANEL:Notify(text, duration, col)
    ...
    col = col or lia.color.getCurrentTheme().theme
    ...
    mp.Paint = function(_, w, h)
        ...
        draw.SimpleText(text, 'Fated.20', w * 0.5, h * 0.5 - 1, lia.color.text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
...
function PANEL:Paint(w, h)
    RNDX().Rect(0, 0, w, h):Rad(6):Color(lia.color.window_shadow):Shadow(10, 16):Shape(RNDX.SHAPE_IOS):Draw()
    if not self.bool_lite then RNDX().Rect(0, 0, w, 24):Radii(6, 6, 0, 0):Color(lia.color.header):Draw() end
    local headerTall = self.bool_lite and 0 or 24
    if self.bool_alpha and lia.ui.convar.blur then RNDX().Rect(0, headerTall, w, h - headerTall):Radii(self.bool_lite and 6 or 0, self.bool_lite and 6 or 0, 6, 6):Blur():Draw() end
    RNDX().Rect(0, headerTall, w, h - headerTall):Radii(self.bool_lite and 6 or 0, self.bool_lite and 6 or 0, 6, 6):Color((self.bool_alpha and lia.ui.convar.blur) and lia.color.background_alpha or lia.color.background):Draw()
    if not self.bool_lite then
        if self.center_title ~= '' then draw.SimpleText(self.center_title, 'Fated.20b', w * 0.5, 12, lia.color.header_text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER) end
        draw.SimpleText(self.title, 'Fated.16', 6, 4, lia.color.header_text)
    end
end
