...
function PANEL:Paint(w, h)
    ...
    local minmaxPadY = 12
    draw.SimpleText(self.text, textFont, padX, padTop, lia.color.text)
    ...
    RNDX().Rect(barStart, barY, barW, barH):Rad(barR):Color(lia.color.window_shadow):Shadow(5, 20):Draw()
    RNDX.Draw(barR, barStart, barY, barW, barH, lia.color.focus_panel)
    RNDX.Draw(barR, barStart, barY, barW, barH, lia.color.button_shadow)
    ...
    RNDX.Draw(barR, barStart, barY, self.smoothPos, barH, lia.color.getCurrentTheme().theme)
    local handleX = barStart + self.smoothPos
    local handleY = barY + barH / 2
    RNDX.DrawShadows(handleR, handleX - handleW / 2, handleY - handleH / 2, handleW, handleH, lia.color.window_shadow, 3, 10)
    ...
    local colorText = Color(currentTheme.theme.r, currentTheme.theme.g, currentTheme.theme.b, math.floor(self._dragAlpha))
    ...
    draw.SimpleText(self.min_value, minmaxFont, barStart, barY + barH + minmaxPadY - 4, lia.color.gray, TEXT_ALIGN_LEFT)
    draw.SimpleText(self.max_value, minmaxFont, barEnd, barY + barH + minmaxPadY - 4, lia.color.gray, TEXT_ALIGN_RIGHT)
end
