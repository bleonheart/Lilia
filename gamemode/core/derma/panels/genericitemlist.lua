...
function PANEL:SetTitle(title)
    self.Title = title and L(title) or "Item List"
    if self:GetTitle() ~= (title and L(title) or "Item List") then DFrame.SetTitle(self, title and L(title) or "Item List") end
end
...
function PANEL:Paint(w, h)
    ...
    draw.SimpleText(self.Title or "Item List", "LiliaFont.25", w / 2, 25, lia.color.theme.text or Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
...
function PANEL:SetTitle(title)
    self.Title = title and L(title) or "Select Items"
    if self:GetTitle() ~= (title and L(title) or "Select Items") then DFrame.SetTitle(self, title and L(title) or "Select Items") end
end
...
function PANEL:Paint(w, h)
    surface.SetDrawColor(lia.color.theme.background or Color(45, 45, 45, 250))
    surface.DrawRect(0, 0, w, h)
    draw.SimpleText(self.Title or "Select Items", "LiliaFont.25", w / 2, 25, lia.color.theme.text or Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
end
