...
function PANEL:Paint(w, h)
    ...
    if optionCount > 0 then
        ...
        for i, option in ipairs(currentOptions) do
            ...
            if showDesc then
                local descLines, descHeight = self:WrapText(option.desc, self.descFont, math_min(175 * self.scale, ringWidth * 1.35))
...
function PANEL:DrawCenterContent(centerX, centerY, radius, title, desc, textColor, alpha, iconMaterial, actionText)
    ...
    local titleLines, titleHeight = self:WrapText(title or "", self.titleFont, maxWidth)
    local descLines, descHeight = self:WrapText(desc or "", self.descFont, maxWidth * 0.95)
...
function PANEL:WrapText(text, font, maxWidth)
