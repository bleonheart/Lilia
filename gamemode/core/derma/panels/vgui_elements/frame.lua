...
function PANEL:Init()
    ...
    self.cls.DoRightClick = function()
        ...
        DM:AddOption('Прозрачность', function() self.bool_alpha = not self.bool_alpha end, self.bool_alpha and 'icon16/bullet_green.png' or 'icon16/bullet_red.png')
        local boolInput = self:IsKeyboardInputEnabled()
        DM:AddOption('Передвижение из меню', function() self:SetKeyBoardInputEnabled(not boolInput) end, not boolInput and 'icon16/bullet_green.png' or 'icon16/bullet_red.png')
        DM:AddOption('Закрыть окно', function() self:Remove() end, 'icon16/cross.png')
    end
end
...
local flagsHeader = lia.rndx.NO_BL + lia.rndx.NO_BR
local flagsBackground = lia.rndx.NO_TL + lia.rndx.NO_TR
function PANEL:Paint(w, h)
    ...
    if self.bool_alpha then lia.rndx.Rect(0, headerTall, w, h - headerTall):Radii(self.bool_lite and 6 or 0, self.bool_lite and 6 or 0, 6, 6):Blur():Draw() end
