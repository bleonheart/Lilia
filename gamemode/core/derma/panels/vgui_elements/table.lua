...
function PANEL:CreateRow(rowIndex, rowData)
    ...
    row.Paint = function(s, w, h)
        local bgColor = self.selectedRow == rowIndex and lia.color.theme.theme or
                       (s:IsHovered() and lia.color.theme.hover or lia.color.theme.panel_alpha[1])
        lia.rndx.Rect(0, 0, w, h)
            :Color(bgColor)
            :Shape(lia.rndx.SHAPE_IOS)
        :Draw()
...
function PANEL:Paint(w, h)
    lia.rndx.Rect(0, 0, w, h)
        :Rad(16)
        :Color(lia.color.theme.panel_alpha[2])
        :Shape(lia.rndx.SHAPE_IOS)
    :Draw()
end
