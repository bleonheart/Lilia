...
local function buildSkinPreview()
    ...
    skinTestFrame:SetTitle(L("liliaSkinPreview"))
    ...
    addRow(list, L("skinPreviewCheckboxesRadio"), 70, function(parent)
        local cb = parent:Add("DCheckBoxLabel")
        cb:Dock(LEFT)
        cb:SetText(L("checkbox"))
        cb:SetValue(true)
        cb:SizeToContents()
    ...
    addRow(list, L("skinPreviewComboBox"), 70, function(parent)
        local combo = parent:Add("DComboBox")
        combo:Dock(LEFT)
        combo:SetWide(220)
        combo:SetValue(L("chooseOption"))
        combo:AddChoice(L("optionA"))
        combo:AddChoice(L("optionB"))
        combo:AddChoice(L("optionC"))
    end)

    addRow(list, L("skinPreviewHorizontalScrollbar"), 80, function(parent)
        local scrollBar = parent:Add("liaHorizontalScroll")
        scrollBar:Dock(FILL)
        for i = 1, 12 do
            local btn = vgui.Create("DButton")
            btn:SetWide(120)
            btn:SetText(L("itemNumber", i))
            scrollBar:AddItem(btn)
        end
    end)

    addRow(list, L("skinPreviewNumSlider"), 80, function(parent)
        local slider = parent:Add("DNumSlider")
        slider:Dock(TOP)
        slider:SetText(L("volume"))
        slider:SetMin(0)
        slider:SetMax(100)
    ...
    addRow(list, L("skinPreviewProgressBar"), 70, function(parent)
    ...
    addRow(list, L("skinPreviewListViewSelection"), 120, function(parent)
        local listView = parent:Add("DListView")
        listView:Dock(FILL)
        listView:AddColumn(L("name"))
        listView:AddColumn(L("role"))
        listView:AddLine(L("sampleNameAlex"), L("sampleRoleBuilder"))
        listView:AddLine(L("sampleNameSam"), L("sampleRolePilot"))
        listView:AddLine(L("sampleNameJamie"), L("sampleRoleEngineer"))
        listView:SelectItem(listView:GetLine(2))
    end)

    addRow(list, L("skinPreviewListBox"), 100, function(parent)
        local listBox = parent:Add("DListBox")
        listBox:Dock(FILL)
        listBox:AddItem(L("firstChoice"))
        listBox:AddItem(L("secondChoice"))
        listBox:AddItem(L("thirdChoice"))
    end)

    addRow(list, L("skinPreviewMenuTooltip"), 70, function(parent)
        local btn = parent:Add("DButton")
        btn:Dock(LEFT)
        btn:SetWide(200)
        btn:SetText(L("openTestMenu"))
        btn:SetTooltip(L("hoverTooltipStyling"))
        btn.DoClick = function()
            local menu = DermaMenu()
            menu:AddOption(L("primaryOption"))
            menu:AddSpacer()
            local submenu = menu:AddSubMenu(L("subMenu"))
            submenu:AddOption(L("childA"))
            submenu:AddOption(L("childB"))
            menu:Open()
        end
    end)

    addRow(list, L("textEntry"), 60, function(parent)
        local textEntry = parent:Add("DTextEntry")
        textEntry:Dock(FILL)
        textEntry:SetPlaceholderText(L("enterTextHere"))
        textEntry:SetText(L("sampleText"))
    end)

    addRow(list, L("treeView"), 120, function(parent)
        local tree = parent:Add("DTree")
        tree:Dock(FILL)
        local node1 = tree:AddNode(L("rootNode"), "icon16/folder.png")
        node1:AddNode(L("childNodeOne"), "icon16/page.png")
        node1:AddNode(L("childNodeTwo"), "icon16/page.png")
        local node4 = tree:AddNode(L("anotherRoot"), "icon16/folder.png")
        node4:AddNode(L("anotherChild"), "icon16/page.png")
    end)

    addRow(list, L("propertySheetWithTabs"), 120, function(parent)
        ...
        label1:SetText(L("firstTabContent"))
        label1:SetPos(20, 20)
        local panel2 = vgui.Create("DPanel")
        panel2:SetPaintBackground(false)
        local label2 = panel2:Add("DLabel")
        label2:SetText(L("secondTabContent"))
        label2:SetPos(20, 20)
        sheet:AddSheet(L("tabOne"), panel1, "icon16/page.png")
        sheet:AddSheet(L("tabTwo"), panel2, "icon16/page_white.png")
    end)

    addRow(list, L("categoryList"), 100, function(parent)
        local catList = parent:Add("DCategoryList")
        catList:Dock(FILL)
        local cat1 = catList:Add(L("categoryOne"))
        cat1:Add(L("itemOneOne"))
        cat1:Add(L("itemOneTwo"))
        local cat2 = catList:Add(L("categoryTwo"))
        cat2:Add(L("itemTwoOne"))
        cat2:Add(L("itemTwoTwo"))
    end)

    addRow(list, L("verticalScrollbar"), 100, function(parent)
        ...
        for i = 1, 8 do
            ...
            btn:SetText(L("scrollItemNumber", i))
        end
    end)

    addRow(list, L("buttonsVariousStates"), 70, function(parent)
        local normalBtn = parent:Add("DButton")
        normalBtn:Dock(LEFT)
        normalBtn:SetWide(100)
        normalBtn:SetText(L("normal"))
        ...
        iconBtn:SetWide(100)
        iconBtn:SetText(L("iconButton"))
    end)

    addRow(list, L("spawnIcons"), 80, function(parent)
    ...
    addRow(list, L("frameWithWindowControls"), 140, function(parent)
        local frame = parent:Add("DFrame")
        frame:SetSize(300, 100)
        frame:Dock(LEFT)
        frame:SetTitle(L("testFrame"))
        frame.btnMinim:SetEnabled(true)
        frame.btnMaxim:SetEnabled(true)
    end)

    addRow(list, L("basicPanel"), 80, function(parent)
        local panel = parent:Add("DPanel")
        panel:Dock(FILL)
        panel:SetBackgroundColor(Color(50, 50, 50, 100))
        local label = panel:Add("DLabel")
        label:SetText(L("basicPanel"))
        label:Center()
    end)

    addRow(list, L("numberSpinnerUpDown"), 80, function(parent)
    ...
    addRow(list, L("menuBar"), 60, function(parent)
        local menuBar = parent:Add("DMenuBar")
        menuBar:Dock(FILL)
        local menu = menuBar:AddMenu(L("file"))
        menu:AddOption(L("new"))
        menu:AddOption(L("open"))
        menu:AddOption(L("save"))
        local editMenu = menuBar:AddMenu(L("edit"))
        editMenu:AddOption(L("cut"))
        editMenu:AddOption(L("copy"))
        editMenu:AddOption(L("paste"))
    end)

    addRow(list, L("tabs"), 80, function(parent)
        local sheet = parent:Add("DPropertySheet")
        sheet:Dock(FILL)
        local panel1 = vgui.Create("DPanel")
        panel1:SetPaintBackground(false)
        local label1 = panel1:Add("DLabel")
        label1:SetText(L("tabContentOne"))
        label1:SetPos(10, 10)
        sheet:AddSheet(L("tabOne"), panel1)
    end)

    addRow(list, L("contentPanelAndIcon"), 100, function(parent)
