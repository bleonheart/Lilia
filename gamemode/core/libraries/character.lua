        ...
        lia.db.query(query, function(data)
            ...
            for _, v in ipairs(results) do
                ...
                lia.inventory.loadAllFromCharID(charId):next(function(inventories)
                    ...
                    if lia.devmode then
                        charDevLog(string.format("Character %s restored with %s inventories in %.3fs", tostring(charId), tostring(#inventories), SysTime() - charStarted))
                    end
                    if done == #results and callback then callback(characters) end
                    if done == #results and lia.devmode then
                        charDevLog(string.format("Finished restoring %s character(s) for %s in %.3fs", tostring(#results), steamID, SysTime() - restoreStarted))
                    end
                end)
            end
        end)
