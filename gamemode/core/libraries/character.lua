...
lia.char.registerVar("faction", {
    ...
    onValidate = function(value, _, client)
        ...
        -- Prevent changing to staff faction through any means except menu
        if value == FACTION_STAFF then
            -- Only allow if the client has the createStaffCharacter privilege
            if not client or not client:hasPrivilege("createStaffCharacter") then
                return false, "staffFactionRestricted"
            end
            return true
        end
