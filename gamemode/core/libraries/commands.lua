...
if SERVER then
    ...
    concommand.Add("lia_check_fonts", function(client)
        ...
        -- Extract all unique font names from Lilia font registrations
        local fontsToCheck = {}
        local fontNames = {}
        ...
        -- Add the configured font if it's not already included
        local configFont = lia.config.get("Font") or "Montserrat Medium"
        if not fontNames[configFont] then table.insert(fontsToCheck, configFont) end
        -- Get available system fonts
        local availableSystemFonts = {}
        if system and system.GetFonts then
            local systemFonts = system.GetFonts()
            for _, fontInfo in ipairs(systemFonts) do
                availableSystemFonts[fontInfo] = true
            end
        end
        ...
        for _, fontName in ipairs(fontsToCheck) do
            -- Check if font is in system fonts (case-insensitive match)
            local fontFound = false
            if availableSystemFonts then
                for systemFont, _ in pairs(availableSystemFonts) do
                    if string.lower(systemFont) == string.lower(fontName) or string.find(string.lower(systemFont), string.lower(fontName), 1, true) or string.find(string.lower(fontName), string.lower(systemFont), 1, true) then
                        fontFound = true
                        break
                    end
                end
            else
                -- Fallback: try to create the font and check if it works
                local testFontName = "lia_font_test_" .. fontName:gsub("%s+", "_")
            ...
            if fontFound then
                table.insert(availableFonts, fontName)
                MsgC(Color(0, 255, 0), "✓ " .. fontName .. " - Available\n")
            else
                table.insert(missingFonts, fontName)
                MsgC(Color(255, 0, 0), "✗ " .. fontName .. " - Missing\n")
            end
        end

        MsgC(Color(255, 255, 255), "----------------------------------------\n")
        if #missingFonts > 0 then
            MsgC(Color(255, 255, 0), "Missing fonts (" .. #missingFonts .. "):\n")
            for _, font in ipairs(missingFonts) do
                MsgC(Color(255, 165, 0), "  • " .. font .. "\n")
            end

            MsgC(Color(255, 255, 255), "\n")
        end

        if #availableFonts > 0 then
            MsgC(Color(0, 255, 0), "Available fonts (" .. #availableFonts .. "):\n")
            for _, font in ipairs(availableFonts) do
                MsgC(Color(0, 255, 0), "  • " .. font .. "\n")
            end

            MsgC(Color(255, 255, 255), "\n")
        end

        MsgC(Color(255, 255, 255), "Note: Fonts are checked against system font list or by testing font creation.\n")
        if not availableSystemFonts then MsgC(Color(255, 165, 0), "Warning: system.GetFonts() not available, using fallback font testing method.\n") end
    end)
end