if SERVER then
    ...
    hook.Add("simfphysUse", "liaSimfphys", function(entity, client)
        ...
        if entity.IsBeingEntered then
            client:notifyWarningLocalized("Someone is entering this car!")
            return true
        end
        ...
        timer.Create(timerID, 0.1, 0, function()
            ...
            if client:GetPos():DistToSqr(entity:GetPos()) > 250 * 250 then
                ...
                entity.IsBeingEntered = false
                client:setAction()
                client:notifyWarningLocalized("You are too far away!")
            end
        end)
...
lia.config.add("DisableSimfphysHUD", "Disable simfphys HUD", false, function()
    if SERVER then
        for _, client in player.Iterator() do
            if IsValid(client) then client:notifyInfoLocalized("The simfphys HUD setting will only apply after a Lua refresh or server restart.,") end
        end
    end
end, {
