...
function lia.config.add(key, name, value, callback, data)
    ...
    data.desc = isstring(data.desc) and resolveToken(data.desc) or data.desc
    data.category = isstring(data.category) and resolveToken(data.category) or data.category
    lia.config.stored[key] = {
        name = isstring(name) and resolveToken(name) or name or key,
        data = data,
        value = savedValue,
        default = value,
        desc = data.desc,
        category = data.category or resolveToken("@character"),
...
lia.config.add("CurrencySingularName", "@currencySingularName", "@currencySingular", function(newVal) lia.currency.singular = lia.lang.resolveToken(newVal) end, {
    desc = "@currencySingularNameDesc",
    category = "@Core",
    type = "Generic"
})

lia.config.add("CurrencyPluralName", "@currencyPluralName", "@currencyPlural", function(newVal) lia.currency.plural = lia.lang.resolveToken(newVal) end, {
    ...
    if hook.Run("CanPlayerModifyConfig", LocalPlayer()) ~= false then
        ...
        pages[#pages + 1] = {
            ...
            drawFunc = function(parent)
                ...
                searchEntry:SetPlaceholderText(L("searchConfigs"))
