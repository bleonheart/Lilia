...
function lia.admin.notifyProtectedStaffTarget(admin)
    if IsValid(admin) then admin:notifyErrorLocalized("You cannot use targeted admin commands on players in the staff faction.") end
end
...
function getPrivilegeCategory(privilegeName)
    ...
    elseif lia.command and lia.command.list and lia.command.list[privilegeName] then
        category = lia.lang.resolveToken("Staff Permissions")
    else
...
if SERVER then
    ...
    local function buildPrivilegeList(container, state)
        ...
        local function togglePrivilege(privilege, row, toggle)
            if not state.editable then
                LocalPlayer():notifyErrorLocalized("Base usergroups cannot be edited")
                return
            end
    ...
    local function SetupUserGroupInterface(parent)
        ...
        renameButton.DoClick = function()
            ...
            if lia.admin.DefaultGroups[groupName] then
                LocalPlayer():notifyErrorLocalized("[Lilia Administration] The base usergroups cannot be renamed!")
                return
            end
        ...
        deleteButton.DoClick = function()
            ...
            if not groupName then return end
            if lia.admin.DefaultGroups[groupName] then
                LocalPlayer():notifyErrorLocalized("[Lilia Administration] The base usergroups cannot be removed!")
                return
            end
