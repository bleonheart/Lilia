...
function lia.admin.notifyProtectedStaffTarget(admin)
    if IsValid(admin) then admin:notifyError("You cannot use targeted admin commands on players in the staff faction.") end
end
...
function getPrivilegeCategory(privilegeName)
            ...
            if module.Privileges and istable(module.Privileges) then
                for privID, priv in pairs(module.Privileges) do
                    if privID == privilegeName then
                        category = priv.Category or module.name or "Unassigned"
                        break
                    end
                end
            end
    ...
    if not category and CAMI then
        ...
        if camiPriv and camiPriv.Category then category = camiPriv.Category end
    end
...
function lia.admin.applyPunishment(client, infraction, kick, ban, time, kickKey, banKey)
    ...
    if kick then lia.admin.execCommand("kick", client, nil, kickKey) end
    if ban then lia.admin.execCommand("ban", client, bantime, banKey) end
end
...
function lia.admin.registerPrivilege(priv)
    ...
    lia.admin.privilegeNames[id] = priv.Name or lia.admin.privilegeNames[id] or priv.ID
    ...
    if description ~= "" then
        lia.admin.privilegeDescriptions[id] = description
...
if SERVER then
    function lia.admin.notifyAdmin(notification)
        for _, client in player.Iterator() do
            ...
            if permission then client:notifyAdmin(notification) end
        end
    end
    ...
    function lia.admin.serverExecCommand(cmd, victim, dur, reason, admin)
        ...
        local function notifyAdmin(kind, key, ...)
            ...
            if IsValid(admin) then
                if kind == "error" then
                    admin:notifyError(string.format(key, ...))
                else
                    admin:notifySuccess(string.format(key, ...))
                end
            elseif SERVER then
                print("[Lilia] " .. tostring(key))
            end
        end
        ...
        if not isConsoleAdmin and not lia.admin.hasAccess(admin, privilegeID) then
            notifyAdmin("error", "You do not have permission to use this command.")
        ...
        if not IsValid(target) then
            notifyAdmin("error", "Target not found.")
            return false
        end

        if lia.admin.isProtectedStaffTarget(cmd, target) then
            notifyAdmin("error", "You cannot use targeted admin commands on players in the staff faction.")
            return false
        end
        ...
        if cmd == "kick" then
            ...
            notifyAdmin("success", "Player kicked.")
        ...
        elseif cmd == "ban" then
            ...
            notifyAdmin("success", "Player banned.")
        ...
        elseif cmd == "unban" then
            ...
            if steamid and steamid ~= "" then
                ...
                notifyAdmin("success", "Player unbanned")
        ...
        elseif cmd == "mute" then
            if target:getChar() then
                ...
                notifyAdmin("success", "Player muted successfully.")
        ...
        elseif cmd == "unmute" then
            if target:getChar() then
                ...
                notifyAdmin("success", "Player unmuted successfully.")
        ...
        elseif cmd == "gag" then
            ...
            notifyAdmin("success", "Player gagged successfully.")
        ...
        elseif cmd == "ungag" then
            ...
            notifyAdmin("success", "Player ungagged successfully.")
        ...
        elseif cmd == "freeze" then
            ...
            notifyAdmin("success", "%s frozen successfully.", target:Name())
            logAdminAction("plyFreeze", target:Name(), duration)
            staffAction("FREEZE", adminName .. " froze " .. targetInfo)
            return true
        elseif cmd == "unfreeze" then
            target:Freeze(false)
            notifyAdmin("success", "%s unfrozen successfully.", target:Name())
        ...
        elseif cmd == "slay" then
            ...
            notifyAdmin("success", "Player killed.")
        ...
        elseif cmd == "bring" then
            ...
            notifyAdmin("success", "%s brought successfully.", target:Name())
        ...
        elseif cmd == "goto" then
            ...
            notifyAdmin("success", "Teleported to %s successfully.", target:Name())
        ...
        elseif cmd == "return" then
            ...
            if pos then
                ...
                notifyAdmin("success", "%s returned successfully.", IsValid(target) and target:Name() or adminName)
        ...
        elseif cmd == "jail" then
            target:Lock()
            target:Freeze(true)
            notifyAdmin("success", "%s jailed successfully.", target:Name())
        ...
        elseif cmd == "ignite" then
            ...
            notifyAdmin("success", "%s ignited successfully.", target:Name())
        ...
        elseif cmd == "respawn" then
            ...
            notifyAdmin("success", "%s respawned successfully.", target:Name())
        ...
        elseif cmd == "blind" then
            ...
            notifyAdmin("success", "%s blinded successfully.", target:Name())
        ...
        elseif cmd == "unblind" then
            ...
            net.WriteBool(false)
            net.Send(target)
            notifyAdmin("success", "%s unblinded successfully.", target:Name())
    ...
    local function getPrivilegeDescription(name)
        ...
        for _, module in pairs(lia.module.list or {}) do
            ...
            if moduleDescription ~= nil then
                local description = string.Trim(tostring(moduleDescription or ""))
                if description ~= "" and description ~= tostring(privilegeID) then return description end
            end
        end
        ...
        if CAMI and CAMI.GetPrivilege then
            ...
            if camiDescription ~= nil then
                local description = string.Trim(tostring(camiDescription or ""))
                if description ~= "" and description ~= tostring(privilegeID) then return description end
            end
        end
    ...
    local function buildPrivilegeList(container, state)
        ...
        local function togglePrivilege(privilege, row, toggle)
            if not state.editable then
                LocalPlayer():notifyError("Base usergroups cannot be edited")
                return
            end
        ...
        local function addPrivilegeRow(parent, privilege)
            ...
            row.Paint = function(button, w, h)
                ...
                local title = lia.util.wrapText(getPrivilegeDisplayName(privilege), maxTextWidth, "LiliaFont.17", 1, "...")[1] or ""
                local description = lia.util.wrapText(getPrivilegeDescription(privilege), maxTextWidth, "LiliaFont.15", 1, "...")[1] or ""
