...
function lia.char.registerVar(key, data)
    ...
    if SERVER and data.field and data.fieldType and lia.db and lia.db.tablesLoaded then
        local columnType = data.fieldType == "string" and ("VARCHAR(" .. tostring(data.length or 255) .. ")") or data.fieldType
        lia.db.createColumn("characters", data.field, columnType, data.default):catch(function(message) lia.error("Failed to add character field: " .. tostring(message)) end)
    end
end
...
function lia.char.getCharData(charID, key)
    ...
    return lia.db.select({"key", "value"}, "chardata", {
        ...
        for _, row in ipairs(result.results) do
            ...
            if ok and istable(decoded) then data[row.key] = decoded[1] end
        end
    ...
    function lia.char.restore(client, callback, id)
        ...
        lia.db.query(query, function(data)
            ...
            for _, v in ipairs(results) do
                ...
                local function restoreInventories()
                ...
                lia.char.getCharData(charId):next(function(storedData)
                    character.dataVars = storedData
                    restoreInventories()
                end):catch(function(message)
                    character.dataVars = {}
                    lia.error(string.format("Failed to load character data for %s: %s", tostring(charId), tostring(message)))
                    restoreInventories()
                end)
            end
        end)
