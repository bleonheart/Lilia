...
--[[
    ...
    Example Usage:
        ```lua
        lia.config.add("ExampleEnabled", "Example Enabled", true, nil, {
            desc = "Whether the example feature is enabled.",
            category = "Core",
            type = "Boolean"
        })
        ```
...
--[[
    ...
    Example Usage:
        ```lua
        for _, option in pairs(lia.config.getOptions("DermaSkin")) do
...
if SERVER then
    ...
    net.Receive("liaCfgSet", function(_, client)
        ...
        if type(config.default) == type(value) and hook.Run("CanPlayerModifyConfig", client, key) ~= false then
            ...
            client:notifySuccessLocalized("%s has set \"%s\" to %s.", client:Name(), name or config.name or key, tostring(value))
        end
    end)
else
...
lia.config.add("CurrencySingularName", "Currency Singular Name", "dollar", function(newVal) lia.currency.singular = L(newVal) end, {
    desc = "Singular name of the in-game currency.",
    category = "Core",
    type = "Generic"
})

lia.config.add("CurrencyPluralName", "Currency Plural Name", "dollars", function(newVal) lia.currency.plural = L(newVal) end, {
