...
if SERVER then
    ...
    function lia.dialog.openDialog(client, npc, npcID)
        ...
        if not npcData then
            client:notifyWarningLocalized("This NPC type is not registered. Please select a valid NPC type.")
        ...
        modelEntry.action = function(value)
            if IsValid(npc) and value and value ~= "" then
                ...
                if oldModel ~= value then
                    ...
                    LocalPlayer():notifySuccessLocalized("NPC model updated to: %s. Bodygroups and skin have been reset.", value)
                end
            end
        end
        ...
        if hasAnimations then
            ...
            refreshBtn.DoClick = function()
                if IsValid(npc) then
                    ...
                    if sequences and #sequences > 0 then
                        ...
                        animationCombo:ChooseOptionData(selectedAnimation)
                        LocalPlayer():notifySuccessLocalized("Animation list refreshed! Found %s animations.", #sequences)
                    else
                        LocalPlayer():notifyErrorLocalized("No animations found for this model.")
                    end
                end
            end
        else
            ...
            refreshAnimBtn.DoClick = function()
                if IsValid(npc) then
                    local sequences = npc:GetSequenceList()
                    if sequences and #sequences > 0 then
                        LocalPlayer():notifySuccessLocalized("Found %s animations! Please reopen the customization menu.", #sequences)
                    else
                        LocalPlayer():notifyErrorLocalized("Still no animations found. The model might not have animations.")
                    end
                end
            end
        end
        ...
        if lia.dialog.pendingOtherConfigs and #lia.dialog.pendingOtherConfigs > 0 then
            ...
            for _, config in ipairs(otherConfigs) do
                if isfunction(config.onOpen) then
                    ...
                    configBtn:SetText(lia.lang.resolveToken(config.name or config.id) or "Configuration")
...
function lia.dialog.openConfigurationPicker(npc, npcID)
    ...
    if #configurations == 0 then
        LocalPlayer():notifyErrorLocalized("No NPC configurations are available.")
        return
    end
...
if SERVER then
        ...
        onApply = function(ply, npc, customData)
            ...
            if customData.dialogType then
                ...
                if dialogType == "" then
                    ...
                    hook.Run("SaveData")
                    ply:notifySuccessLocalized("NPC dialog type updated successfully!")
                    return
                end
    ...
    lia.dialog.registerConfiguration("appearance", {
        onApply = function(ply, npc, customData)
            ...
            hook.Run("UpdateEntityPersistence", npc)
            hook.Run("SaveData")
            ply:notifySuccessLocalized("NPC customized successfully!")
        end
    })
