...
function lia.dialog.getCompatibleDialogOptions(npc)
    local options = {}
    for uniqueID, data in pairs(lia.dialog.stored or {}) do
        if lia.dialog.isConversationDialogData(data) and lia.dialog.isDialogCompatibleWithEntity(npc, data) then options[#options + 1] = {data.PrintName or uniqueID, uniqueID} end
    end
