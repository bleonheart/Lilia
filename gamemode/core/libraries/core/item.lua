    ...
    take = {
        ...
        onRun = function(item)
            ...
            inventory:add(item):next(function()
                ...
                if err == "noFit" then
                    client:notifyErrorLocalized("This item can't fit in your inventory. (%sx%s)", item:getWidth(), item:getHeight())
                else
        ...
        onRun = function(item, data)
            ...
            if inv and x and y then
                ...
                if x < 1 or y < 1 or x + w - 1 > invW or y + h - 1 > invH then
                    if showNoFitNotice and IsValid(item.player) then item.player:notifyErrorLocalized("This item can't fit in your inventory. (%sx%s)", w, h) end
                    return false
                end
    ...
    giveForward = {
        ...
        onRun = function(item)
            ...
            if not targetInv:doesFitInventory(item) then
                client:notifyLocalized("This item can not fit in your inventory.")
                return false
            end
            ...
            target:requestBinaryQuestion(string.format("%s wants to give you a %s. Accept?", client:Name(), item.name), "Yes", "No", function(choice)
                if choice == 0 then
                    ...
                    client:notifyLocalized("%s declined your item offer.", target:Name())
                    target:notifyLocalized("You declined %s's item offer.", client:Name())
                end
            end)
            return false
        end,
