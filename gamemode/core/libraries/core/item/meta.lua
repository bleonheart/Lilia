...
function ITEM:getQuantity()
    if self.id == 0 then return self.maxStack end
    return self.quantity
end

function ITEM:paintOver(item)
    if self.maxStack <= 1 then return end
...
function ITEM:canCombine(other)
    return self.maxStack > 1 and other.maxStack > 1 and self.uniqueID == other.uniqueID and not self:getData("equip", false) and not other:getData("equip", false)
end

function ITEM:onCombine(other)
    if not self:canCombine(other) then return end
    local combined = self:getQuantity() + other:getQuantity()
    if combined <= self.maxStack then
        self:setQuantity(combined)
        other:remove()
    else
        self:setQuantity(self.maxStack)
        other:setQuantity(combined - self.maxStack)
    end
    return true
end
...
function ITEM:getDesc()
    if self.maxStack > 1 then return string.format("Amount: %s", self:getQuantity()) end
    return self.desc
end
...
if SERVER then
    ...
    function ITEM:interact(action, client, entity, data)
        ...
        if canInteract == false then
            if reason then client:notifyError(reason) end
            return false
        end
