...
--[[
    ...
    Example Usage:
        ...
        lia.keybind.add("toggleExample", {
            keyBind = KEY_F6,
            desc = "Toggles the example feature.",
            category = "misc",
    ...
    onPress = function(client)
        ...
        if client:isStaffOnDuty() then
            ...
            if oldCharID > 0 then
                ...
                client:notifyErrorLocalized("No previous character to swap to.")
            end
        else
                ...
                if canCreateStaffCharacter then
                    ...
                    lia.char.create(staffCharData, function(charID)
                        if IsValid(client) and charID then
                            ...
                            client:notifySuccessLocalized("A staff character has been automatically created for you.")
                        end
                    end)
                else
...
lia.keybind.add("convertEntity", {
    ...
    onPress = function(client)
        ...
        if trace.HitPos:Distance(client:GetPos()) > 200 then
            client:notifyErrorLocalized("Entity is too far away.")
            return
        end

        if targetEntity:IsPlayer() or targetEntity:isItem() or targetEntity:GetClass() == "lia_money" then
            client:notifyErrorLocalized("Cannot convert this entity.")
            return
        end
        ...
        if not hasItemDefinition then
            client:notifyErrorLocalized("This entity type cannot be converted to an item.")
            return
        end
        ...
        if not inventory:canAdd(itemUniqueID) then
            client:notifyErrorLocalized("No space available for the item.")
            return
        end

        inventory:add(itemUniqueID):next(function(item)
            client:notifyLocalized("Successfully converted entity to item: %s", item:getName())
