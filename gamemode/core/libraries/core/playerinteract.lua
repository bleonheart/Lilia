...
if SERVER then
            ...
            client:requestString("Give Money", "Enter amount...", function(amount)
                ...
                if originalAmount ~= amount and originalAmount > 0 then
                    ...
                    for _, admin in player.Iterator() do
                        if admin:IsAdmin() then admin:notifyLocalized("%s attempted to %s with decimal amount %s (floored to %s) - potential money duping!", client:Name(), "givemoney", tostring(originalAmount), tostring(amount)) end
                    end
                end

                if not amount or amount <= 0 then
                    client:notifyErrorLocalized("Invalid amount.")
                    return
                end

                if not IsValid(client) or not client:getChar() then return end
                if client:isFamilySharedAccount() and not lia.config.get("AltsDisabled", false) then
                    client:notifyErrorLocalized("You cannot transfer or drop money with a family-shared account")
                    return
                end
                ...
                client:notifyMoneyLocalized("You transferred %s to %s", lia.currency.get(amount), targetName)
                target:notifyMoneyLocalized("You received %s from %s", lia.currency.get(amount), senderName)
    ...
    lia.playerinteract.addAction("changeToWhisper", {
        ...
        onRun = function(client)
            ...
            client:notifyInfoLocalized("Voice range set to %s.", "Whispering")
    ...
    lia.playerinteract.addAction("changeToTalk", {
        ...
        onRun = function(client)
            ...
            client:notifyInfoLocalized("Voice range set to %s.", "Talking")
            hook.Run("OnVoiceTypeChanged", client)
        end,
        serverOnly = true
    })
    ...
    lia.playerinteract.addAction("changeToYell", {
        ...
        onRun = function(client)
            ...
            client:notifyInfoLocalized("Voice range set to %s.", "Yelling")
            hook.Run("OnVoiceTypeChanged", client)
        end,
        serverOnly = true
    })
    ...
    Example Usage:
        ```lua
        lia.playerinteract.openMenu(options, true, "Interaction Menu", KEY_TAB, "liaRunInteraction", true)
        ```
