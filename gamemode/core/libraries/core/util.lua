...
function lia.util.findPlayer(client, identifier)
    ...
    if not isstring(identifier) or identifier == "" then
        if isValidClient then client:notifyErrorLocalized("Must Provide a String") end
        return nil
    end
    ...
    if string.match(identifier, "^STEAM_%d+:%d+:%d+$") then
        ...
        if isValidClient then client:notifyErrorLocalized("Player does not exist.") end
        return nil
    end
    ...
    if isValidClient and identifier == "@" then
        ...
        client:notifyErrorLocalized("You need to be looking at someone to use '@'")
        return nil
    end
