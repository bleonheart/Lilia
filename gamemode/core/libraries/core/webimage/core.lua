...
local function validateURL(url)
    if not isstring(url) then return false, "URL Not Valid String" end
    ...
    domain = domain:lower()
    if domain == "localhost" or domain:find("^127%.") then return false, "Localhost URLs Not Allowed" end
    if domain:match("^%d+%.%d+%.%d+%.%d+$") then
        ...
        for _, part in ipairs(parts) do
            ...
            if not number or number < 0 or number > 255 then return false, "invalid IP address octet" end
        end
    else
...
local function saveImage(name, data, flags, cb, downloaded, source)
    ...
    if not extension then
        ...
        return nil, false, "Invalid file format - not PNG or JPEG"
    end
