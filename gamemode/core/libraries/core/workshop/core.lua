            ...
            drawFunc = function(parent)
                ...
                local function mountRecord(record, notifyPlayer, callback)
                    ...
                    local function mountedSuccessfully(path, files, stage)
                        ...
                        updateStats()
                        if notifyPlayer then LocalPlayer():notify(string.format("Workshop addon downloaded: %s", record.title or record.id)) end
                    ...
                    steamworks.DownloadUGC(record.id, function(path)
                        ...
                        if not isstring(path) or path == "" then
                            ...
                            if notifyPlayer then LocalPlayer():notifyError(string.format("Failed to download workshop addon: %s", record.title or record.id)) end
