...
local function ensurePersistenceColumns(cols)
    ...
    for _, col in ipairs(cols) do
        d = d:next(function() return lia.db.fieldExists("lia_persistence", col) end):next(function(exists) if not exists then return lia.db.createColumn("persistence", col, "text") end end)
    end
    return d
end
