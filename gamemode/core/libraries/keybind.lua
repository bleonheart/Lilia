...
if CLIENT then
    ...
    function PANEL:openMultiActionMenu(actionKey, action, item)
        ...
        for optionKey, option in pairs(options) do
            if isfunction(option) then
                ...
                menu:AddOption(L(optionKey), buildStandaloneInventoryActionInvoker(self, actionKey, action, item, subOption, optionKey), "icon16/brick.png")
            elseif istable(option) then
                ...
                if canRun then
                    ...
                    menu:AddOption(L(subOption.name), buildStandaloneInventoryActionInvoker(self, actionKey, action, item, subOption, optionKey), subOption.icon or "icon16/brick.png")
                end
            end
        end
    ...
    function PANEL:refreshActionButtons()
        ...
        if primary then
            ...
            local label = L(action.name or primary.key)
            local isMulti = action.isMulti or action.multiOptions and (istable(action.multiOptions) or isfunction(action.multiOptions))
        ...
        if drop then
            ...
            self.dropAction:SetText(L(action.name or drop.key))
...
if CLIENT then
    concommand.Add("lia_inventory", toggleStandaloneInventory)
...
hook.Add("PreLiliaLoaded", "liaMenuTabKeybinds", function()
    ...
    for tabKey, tabData in pairs(tabs) do
        ...
        local description = L("openMenuTabDesc", localizeKeybindLabel(tabName))
