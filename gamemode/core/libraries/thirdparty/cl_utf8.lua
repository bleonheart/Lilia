...
local function utf8len(s)
    if not isstring(s) then error(string.format("bad argument #1 to 'utf8len' (string expected, got %s)", type(s))) end
    local pos = 1
    local bytes = s:len()
    local len = 0
    while pos <= bytes do
...
local function utf8sub(s, i, j)
    j = j or -1
    if not isstring(s) then error(string.format("bad argument #1 to 'utf8sub' (string expected, got %s)", type(s))) end
    if not isnumber(i) then error(string.format("bad argument #2 to 'utf8sub' (number expected, got %s)", type(i))) end
    if not isnumber(j) then error(string.format("bad argument #3 to 'utf8sub' (number expected, got %s)", type(j))) end
    local pos = 1
    local bytes = s:len()
    local len = 0
...
local function utf8replace(s, mapping)
    if not isstring(s) then error(string.format("bad argument #1 to 'utf8replace' (string expected, got %s)", type(s))) end
    if not istable(mapping) then error(string.format("bad argument #2 to 'utf8replace' (table expected, got %s)", type(mapping))) end
    local pos = 1
    local bytes = s:len()
    local charbytes
    local newstr = ""
...
local function utf8reverse(s)
    if not isstring(s) then error(string.format("bad argument #1 to 'utf8reverse' (string expected, got %s)", type(s))) end
    local bytes = s:len()
    local pos = bytes
    local charbytes
    local newstr = ""
