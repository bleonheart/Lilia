...
NP.want_trace = false
NP.targets = {
    all = true -- if all=true, catch for all
}
...
local function logSend(targets)
    ...
    elseif IsValid(targets) then
        if shouldTrackPly(targets) then
            tlist[1] = plyKey(targets)
            addStat(targets, cur.name, bytes)
        end
    else
    ...
    if NP.want_trace then ev.stack = debug.traceback("", 3) end
...
concommand.Add("netprof_start", function(ply, _, args)
    ...
    print(("[NetProf] START: target=%s, dur=%ss, threshold=%dB, trace=%s"):format(who == "all" and "ALL" or (IsValid(who) and plyKey(who) or "ALL"), seconds, NP.threshold, tostring(NP.want_trace)))
end)
