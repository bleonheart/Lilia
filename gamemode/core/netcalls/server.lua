...
--[[
    ...
    Example Usage:
        ...
        hook.Add("PostPlayerLoadedChar", "liaExamplePostPlayerLoadedChar", function(client, character, currentChar)
            if currentChar and currentChar:getID() ~= character:getID() then
                client:notifySuccessLocalized("Character loaded: %s", character:getName())
            end
        end)
        ```
...
net.Receive("liaWeaponOverrideUpdate", function(len, ply)
    ...
    ply:notifyLocalized("Successfully updated %s for %s", key, className)
...
net.Receive("liaWeaponRuntimeOverrideUpdate", function(_, ply)
    ...
    ply:notifyLocalized("Successfully updated %s for %s", dotPath, className)
...
net.Receive("liaWeaponRuntimeOverrideReset", function(_, ply)
    ...
    lia.data.set("weaponRuntimeOverrides", lia.item.WeaponRuntimeOverrides, true, true)
    refreshWeaponHolders(className)
    ply:notifyLocalized("Successfully updated %s for %s", "reset", className)
    net.Start("liaWeaponRuntimeOverrideSync")
    net.WriteBool(false)
    net.WriteString(className)
    net.WriteString("")
...
net.Receive("liaKickCharacter", function(_, client)
    ...
    for _, target in player.Iterator() do
        ...
        if targetChar and targetChar:getID() == characterID and (canManageAny or canKick and char and targetChar:getFaction() == char:getFaction()) then
            ...
            target:notifyWarningLocalized("You were kicked from your faction!")
...
net.Receive("liaCheckHack", function(_, client)
    ...
    if IsValid(client) then
        ...
        client:notifyErrorLocalized("Cheating detected. Staff have been notified.")
        for _, p in player.Iterator() do
            ...
            if permission then p:notifyWarningLocalized("%s (%s) was flagged for cheating.", client:Name(), client:SteamID()) end
        end
...
net.Receive("liaTeleportToEntity", function(_, client)
    ...
    if not client:hasPrivilege("teleportToEntity") then
        client:notifyErrorLocalized("No privilege")
        return
    end

    local entity = net.ReadEntity()
    if not IsValid(entity) then
        client:notifyErrorLocalized("Invalid entity selected.")
        return
    end
    ...
    client:notifySuccessLocalized("Teleported to entity")
...
net.Receive("liaMessageData", function(_, client)
    ...
    if charlimit > 0 then
        ...
        if utf8.len(text) > charlimit then
            client:notifyErrorLocalized("Your message has been shortened due to being longer than %s characters!", charlimit)
        else
            if (client.liaNextChat or 0) < CurTime() and text:find("%S") then
                hook.Run("PlayerSay", client, text)
                client.liaNextChat = CurTime() + math.max(#text / 250, 0.4)
...
net.Receive("liaReturnFromEntity", function(_, client)
    if not client.previousPosition then
        client:notifyErrorLocalized("No previous position")
        return
    end
    ...
    client:notifySuccessLocalized("Returned from entity")
    ...
    if isOrdered then
        for _, argInfo in ipairs(spec) do
            ...
            elseif expectedType == "table" then
                ...
                if val == nil then
                    client:notifyErrorLocalized("Please fill in all required fields.")
                    client.liaArgReqs[id] = nil
                    return
                end
            end
...
net.Receive("BodygrouperMenu", function(_, client)
    ...
    if target ~= client then
        ...
        if not permission then
            client:notifyLocalized("No Access")
            return
        end
    else
    ...
    if target:SkinCount() and skn > target:SkinCount() then
        client:notifyLocalized("Invalid skin selection.")
        return
    end
    ...
    if target:GetNumBodyGroups() and target:GetNumBodyGroups() > 0 then
        for k, v in pairs(groups) do
            if v > target:GetBodygroupCount(k) then
                client:notifyLocalized("Invalid bodygroup selection. This often means the model isn't loaded in the server.")
                return
            end
        end
    end
    ...
    if target == client then
        target:notifyLocalized("You changed %s bodygroups.", "your")
    else
        client:notifyLocalized("You changed %s bodygroups.", target:Name() .. "'s")
        target:notifyLocalized("%s changed your bodygroups.", client:Name())
    end
...
net.Receive("WardrobeChangeModel", function(_, client)
    ...
    if not lia.faction.isModelUsable(newModel) then
        client:notifyLocalized("That model isn't allowed.")
        return
    end
    ...
    for _, modelPath in ipairs(validModels) do
        if string.lower(modelPath) == string.lower(newModel) then
            ...
            client:SetupHands()
            client:notifyLocalized("Your model has been updated.")
            return
        end
    end
...
net.Receive("liaGroupsAdd", function(_, p)
    ...
    if lia.admin.DefaultGroups and lia.admin.DefaultGroups[n] then
        p:notifyErrorLocalized("Base usergroups cannot be edited")
        return
    end
    ...
    lia.admin.save()
    broadcastGroups()
    p:notifySuccessLocalized("Group '%s' created.", n)
end)
...
net.Receive("liaGroupsRemove", function(_, p)
    ...
    if n == "" then return end
    if lia.admin.DefaultGroups and lia.admin.DefaultGroups[n] then
        p:notifyErrorLocalized("[Lilia Administration] The base usergroups cannot be removed!")
        return
    end
    ...
    lia.admin.save()
    broadcastGroups()
    p:notifySuccessLocalized("Group '%s' removed.", n)
end)
...
net.Receive("liaGroupsRename", function(_, p)
    ...
    if lia.admin.groups[new] or lia.admin.DefaultGroups and lia.admin.DefaultGroups[new] then
        p:notifyErrorLocalized("[Lilia Administration] The base usergroups cannot be renamed!")
        return
    end
    ...
    broadcastGroups()
    p:notifySuccessLocalized("Group '%s' renamed to '%s'.", old, new)
end)
...
net.Receive("liaGroupsSetPerm", function(_, p)
    ...
    net.WriteBool(value)
    net.Broadcast()
    p:notifySuccessLocalized("Group permissions updated.")
end)
