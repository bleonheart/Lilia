...
net.Receive("liaCharCreate", function(_, client)
    ...
    -- Check if this is a staff character - they bypass character limits
    if not (istable(data) and data.faction == FACTION_STAFF) and hook.Run("CanPlayerCreateChar", client, data) == false then
        return response(nil, "maxCharactersReached")
    end
