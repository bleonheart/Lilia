...
function ENT:setItem(itemID)
    ...
    if not itemTable.temp then
        ...
        local condition = "_schema = " .. lia.db.convertDataType(folder) .. " AND map = " .. lia.db.convertDataType(map) .. " AND _itemID = " .. tonumber(itemID)
...
function ENT:OnRemove()
    ...
    if SERVER and not lia.shuttingDown and self.liaItemID then
        local folder = SCHEMA and SCHEMA.folder or engine.ActiveGamemode()
        local map = game.GetMap()
        local condition = "_schema = " .. lia.db.convertDataType(folder) .. " AND map = " .. lia.db.convertDataType(map) .. " AND _itemID = " .. tonumber(self.liaItemID)
