...
ITEM.functions.Unequip = {
    ...
    onRun = function(item)
        if item:removeAttachment(item.player) then
            item.player:notifySuccessLocalized("Attachment unequipped.")
        else
            item.player:notifyErrorLocalized("Failed to unequip attachment.")
        end
        return false
    end,
...
ITEM.functions.Equip = {
    ...
    onRun = function(item)
        ...
        item.player:notifySuccessLocalized("Attachment equipped.")
        return false
    end,
    onCanRun = function(item) return not IsValid(item.entity) and item:getData("equip") ~= true end
}
