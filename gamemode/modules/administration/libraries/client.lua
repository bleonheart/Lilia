...
local function OpenLogsUI(panel, categorizedLogs)
    ...
    for category, logs in pairs(categorizedLogs) do
        ...
        local function startPopulate(filter)
            filter = string.lower(filter or "")
            if list._populateTimer then
                timer.Remove(list._populateTimer)
                list._populateTimer = nil
            end

            list:Clear()
            local i = 1
            local batchSize = 200
            local timerName = "liaLogsPopulate_" .. tostring(list)
            list._populateTimer = timerName
            timer.Create(timerName, 0, 0, function()
                if not IsValid(list) then
                    ...
                    local added = 0
                    while i <= #logs and added < batchSize do
                        local log = logs[i]
                        local msg = tostring(log.message or "")
                        local sid = tostring(log.steamID or "")
                        local matches = (filter == "")
                            or string.find(string.lower(msg), filter, 1, true)
                            or (sid ~= "" and string.find(string.lower(sid), filter, 1, true))
                ...
                search.OnChange = function() startPopulate(search:GetValue()) end
                    function list:OnRowRightClick(_, line)
...
hook.Add("PopulateAdminTabs", "liaMedalsTab", function(pages)
    if not IsValid(LocalPlayer()) or not LocalPlayer():hasPrivilege("viewPlayerMedals") then return end
    table.insert(pages, {
        name = "medals",
        icon = "icon16/medal_gold_1.png",
        drawFunc = function(panel)
            panelRef = panel
            net.Start("liaRequestMedalsCount")
            net.SendToServer()
        end
    })
end)
...
local medalsCount = 0
local medalsPageSize = 20
local medalsCurrentPage = 1
local medalsTotalPages = 1
local medalsData = {}

net.Receive("liaMedalsCount", function()
    medalsCount = net.ReadInt(32)
    medalsTotalPages = math.ceil(medalsCount / medalsPageSize)
    medalsCurrentPage = 1
    medalsData = {}

    if panelRef and IsValid(panelRef) then
        CreateMedalsUI(panelRef)
    end
end)

function CreateMedalsUI(panel)
    panel:Clear()
    panel:DockPadding(6, 6, 6, 6)
    panel.Paint = nil
    ...
    header.Paint = function(self, w, h)
        lia.derma.rect(0, 0, w, h):Rad(8):Color(Color(40, 40, 40, 200)):Shape(lia.derma.SHAPE_IOS):Draw()
    end

    -- Page info
    local pageInfo = header:Add("DLabel")
    pageInfo:Dock(LEFT)
    pageInfo:DockMargin(10, 0, 10, 0)
    pageInfo:SetTextColor(color_white)
    pageInfo:SetFont("liaMediumFont")
    pageInfo:SizeToContents()

    -- Previous button
    local prevBtn = header:Add("DButton")
    prevBtn:Dock(LEFT)
    prevBtn:DockMargin(0, 5, 5, 5)
    prevBtn:SetWide(80)
    prevBtn:SetText("Previous")
    prevBtn.DoClick = function()
        if medalsCurrentPage > 1 then
            medalsCurrentPage = medalsCurrentPage - 1
            RequestMedalsPage()
        end
    end

    -- Page number input
    local pageInput = header:Add("DTextEntry")
    pageInput:Dock(LEFT)
    pageInput:DockMargin(0, 5, 5, 5)
    pageInput:SetWide(50)
    pageInput:SetNumeric(true)
    pageInput:SetValue(tostring(medalsCurrentPage))
    pageInput.OnEnter = function()
        local newPage = tonumber(pageInput:GetValue()) or 1
        newPage = math.max(1, math.min(medalsTotalPages, newPage))
        medalsCurrentPage = newPage
        RequestMedalsPage()
    end

    -- Next button
    local nextBtn = header:Add("DButton")
    nextBtn:Dock(LEFT)
    nextBtn:DockMargin(0, 5, 5, 5)
    nextBtn:SetWide(80)
    nextBtn:SetText("Next")
    nextBtn.DoClick = function()
        if medalsCurrentPage < medalsTotalPages then
            medalsCurrentPage = medalsCurrentPage + 1
            RequestMedalsPage()
        end
    end
    ...
    -- Page size selector
    local pageSizeLabel = header:Add("DLabel")
    pageSizeLabel:Dock(RIGHT)
    pageSizeLabel:DockMargin(10, 0, 5, 0)
    pageSizeLabel:SetText("Per Page:")
    pageSizeLabel:SetTextColor(color_white)
    pageSizeLabel:SizeToContents()

    local pageSizeCombo = header:Add("DComboBox")
    pageSizeCombo:Dock(RIGHT)
    pageSizeCombo:DockMargin(0, 5, 5, 5)
    pageSizeCombo:SetWide(60)
    pageSizeCombo:AddChoice("10")
    pageSizeCombo:AddChoice("20")
    pageSizeCombo:AddChoice("50")
    pageSizeCombo:AddChoice("100")
    pageSizeCombo:ChooseOptionID(2) -- Default to 20

    local function updatePageSize()
        local newSize = tonumber(pageSizeCombo:GetValue()) or 20
        if newSize ~= medalsPageSize then
            medalsPageSize = newSize
            medalsCurrentPage = 1
            medalsTotalPages = math.ceil(medalsCount / medalsPageSize)
            RequestMedalsPage()
        end
    ...
    pageSizeCombo.OnSelect = updatePageSize

    -- Search box
    local searchPanel = panel:Add("DPanel")
    ...
    search:SetPlaceholderText("Search players/medals...")
    search:SetTextColor(Color(200, 200, 200))
    search.PaintOver = function(_, w, h) lia.derma.rect(0, 0, w, h):Rad(16):Color(Color(0, 0, 0, 100)):Shape(lia.derma.SHAPE_IOS):Draw() end
    ...
    local columns = {
        ...
        {name = "Character", field = "charName"},
        {name = "Medals", field = "medals"},
    ...
    local function populateList(filter)
        ...
        for _, playerData in ipairs(medalsData) do
            local playerName = string.lower(playerData.name or "")
            local charName = string.lower(playerData.charName or "")
            local medalsStr = ""

            -- Build medals string
            if playerData.medals and istable(playerData.medals) then
                local medalNames = {}
                for medalID, state in pairs(playerData.medals) do
                    local medalData = lia.medals.getAll()[medalID]
                    if medalData then
                        table.insert(medalNames, medalData.name .. (state == 2 and " (Worn)" or ""))
                    end
                end
                medalsStr = table.concat(medalNames, ", ")
            end

            local matches = filter == ""
                or playerName:find(filter, 1, true)
                or charName:find(filter, 1, true)
                or string.lower(medalsStr):find(filter, 1, true)

            if matches then
                local line = list:AddLine(
                    playerData.name,
                    playerData.charName,
                    #medalsStr > 50 and string.sub(medalsStr, 1, 47) .. "..." or medalsStr,
                    ""
                )
                ...
                actionsPanel:SetSize(100, 20)

                local viewBtn = actionsPanel:Add("DButton")
                viewBtn:Dock(LEFT)
                viewBtn:DockMargin(2, 2, 2, 2)
                viewBtn:SetWide(40)
                viewBtn:SetText("View")
                viewBtn.DoClick = function()
                    ShowMedalDetails(playerData)
                end

                local takeBtn = actionsPanel:Add("DButton")
                takeBtn:Dock(RIGHT)
                takeBtn:DockMargin(2, 2, 2, 2)
                takeBtn:SetWide(40)
                takeBtn:SetText("Take")
                takeBtn.DoClick = function()
                    ShowTakeMedalMenu(playerData)
                end

                line:SetColumnText(4, "")
                line.Columns[4]:SetContentAlignment(5)
                line.Columns[4]:Dock(FILL)
                line.Columns[4]:SetContent(actionsPanel)
            end
        end
    end

    search.OnChange = function() populateList(search:GetValue()) end

    function RequestMedalsPage()
        pageInput:SetValue(tostring(medalsCurrentPage))
        pageInfo:SetText(string.format("Page %d of %d (%d total)", medalsCurrentPage, medalsTotalPages, medalsCount))
        pageInfo:SizeToContents()

        prevBtn:SetEnabled(medalsCurrentPage > 1)
        nextBtn:SetEnabled(medalsCurrentPage < medalsTotalPages)

        net.Start("liaRequestAllMedals")
        net.WriteInt(medalsCurrentPage, 16)
        net.WriteInt(medalsPageSize, 16)
        net.SendToServer()
    end

    function ShowMedalDetails(playerData)
        local frame = vgui.Create("DFrame")
        frame:SetTitle("Medals for " .. playerData.name)
            ...
            for medalID, state in pairs(playerData.medals) do
                ...
                if medalData then
                    ...
                    panel.Paint = function(self, w, h)
                        lia.derma.rect(0, 0, w, h):Rad(8):Color(Color(40, 40, 40, 200)):Shape(lia.derma.SHAPE_IOS):Draw()
                        if state == 2 then
                            lia.derma.rect(0, 0, w, h):Rad(8):Color(Color(50, 200, 50, 100)):Shape(lia.derma.SHAPE_IOS):Draw()
                        end
                    end
                    ...
                    icon:SetWide(40)
                    icon:SetImage(string.format("medals/%s/%s", lia.medals.getPackID(medalID), medalData.icon))
    ...
    function ShowTakeMedalMenu(playerData)
        if not playerData.medals or not next(playerData.medals) then return end

        local frame = vgui.Create("DFrame")
        frame:SetTitle("Take Medal from " .. playerData.name)
        ...
        for medalID, state in pairs(playerData.medals) do
            ...
            if medalData then
                ...
                btn:SetTall(40)
                btn:SetText(string.format("%s (%s)", medalData.name, state == 2 and "Worn" or "Owned"))
                btn.DoClick = function()
                    net.Start("liaTakeMedalFromPlayer")
                    net.WriteUInt(playerData.charID, 32)
                    net.WriteString(medalID)
                    net.SendToServer()
                    frame:Close()
                end
            end
...
lia.net.readBigTable("liaAllMedalsData", function(data)
    medalsData = data or {}
    if panelRef and IsValid(panelRef) then
        CreateMedalsUI(panelRef)
    end
