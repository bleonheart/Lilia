...
properties.Add("TogglePropBlacklist", {
    ...
    Receive = function(_, _, ply)
        ...
        if table.HasValue(list, model) then
            ...
            ply:notifySuccessLocalized("Removed from blacklist: %s", model)
        else
            table.insert(list, model)
            lia.data.set("prop_blacklist", list, true, true)
            ply:notifySuccessLocalized("Added to blacklist: %s", model)
        end
    end
})
...
lia.command.add("sayall", {
    ...
    onRun = function(client, arguments)
        ...
        if not phrase or phrase == "" then
            client:notifyErrorLocalized("You must provide a phrase to send.")
            return
        end
        ...
        client:notifySuccessLocalized("Sent phrase '%s' to %d chat types.", chatCount, phrase)
    end
})
