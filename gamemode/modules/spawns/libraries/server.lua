...
function MODULE:PlayerDeath(client, inflictor, attacker)
    ...
    if lia.config.get("DeathPopupEnabled", true) then
        ...
        lia.debug("[perm]", "Permission Check for spawns death message self visibility", "isStaffOnDuty=", tostring(client:isStaffOnDuty()), "hasPrivilege(canSeeLogs)=", tostring(client:hasPrivilege("canSeeLogs")), "finalResult=", tostring(isStaff))
