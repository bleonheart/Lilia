...
net.Receive("liaKickCharacterToBase", function(_, client)
    ...
    for _, target in player.Iterator() do
        ...
        if targetChar and targetChar:getID() == characterID then
            ...
            if oldFactionData and oldFactionData.isDefault then
                client:notifyErrorLocalized("Character is already in the base faction.")
                return
            end
            ...
            targetChar:save()
            client:notifySuccessLocalized("%s has been transferred to %s.", target:Name(), defaultFaction.name)
    ...
    if not isOnline then
        lia.db.query("SELECT faction FROM lia_characters WHERE id = " .. characterID):next(function(data)
            ...
            client:notifySuccessLocalized("%s has been transferred to %s.", "Character", defaultFaction.name)
