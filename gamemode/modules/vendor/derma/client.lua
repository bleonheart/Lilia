...
function PANEL:Init()
    ...
    local vendorItemsText = vendorName and (vendorName .. "'s " .. L("adminStickCategoryItems")) or L("vendorItemsTitle")
...
function PANEL:updateCooldown()
    ...
    if self.isSelling then
        if self.action and self.action._hasCustomPaint then
            ...
            self.action:SetVisible(true)
            self.action:SetText(L("doorSell"))
            self.action.text = L("doorSell")
            self.action:SetEnabled(true)
...
function PANEL:updateAction()
    ...
    if not isInCooldown then
        local buttonText = self.isSelling and L("doorSell") or L("buy")
        self.action:SetText(buttonText)
        self.action.text = buttonText
        self.action:SetEnabled(true)
        if self.action._hasCustomPaint then
...
function PANEL:Init()
    ...
    self.name:Dock(TOP)
    self.name:SetPlaceholderText(L("PrintName"))
    ...
    local nameCol = self.items:AddColumn(L("PrintName"))
...
function PANEL:initializeGeneralInfoPanel(entity)
    ...
    if not IsValid(self.nameLabel) then
        ...
        self.nameLabel:SetText(L("PrintName"))
...
function PANEL:showDeletePresetDetails(presetName, presetData)
    ...
    nameLabel:SetText(L("PrintName") .. ": " .. presetName)
    ...
    itemsLabel:SetText(L("adminStickCategoryItems") .. ": " .. table.Count(presetData))
...
function PANEL:showPresetDetails(presetName, presetData)
    ...
    itemsHeader:SetText(L("adminStickCategoryItems"))
    ...
    for itemType, itemData in pairs(presetData) do
        ...
        if item then
            ...
            local modeText = L("discordWarningSystemUnknown")
