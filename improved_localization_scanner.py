#!/usr/bin/env python3
"""
Improved Localization Scanner for Lilia Gamemode
Focuses on finding actual user-facing strings that need localization
"""
...
def find_user_facing_strings():
    """Find strings that are likely user-facing and need localization"""
            ...
            for lua_file in Path(dir_path).rglob("*.lua"):
                try:
                    ...
                    for string in strings:
                        ...
                        # Must contain spaces or be complete sentences/phrases
                        if (' ' in string or
                            string.endswith('.') or
                            string.endswith('!') or
                            string.endswith('?') or
                            # Allow some common UI words
...
def main():
    print("Running improved localization scan...")
    candidates = find_user_facing_strings()

    print(f"Found {len(candidates)} potential user-facing strings")
    ...
    if candidates:
        ...
        with open('improved_candidates.txt', 'w', encoding='utf-8') as f:
            for candidate in candidates:
                f.write(f"'{candidate}'\n")

        print("\nFull list saved to improved_candidates.txt")
    else:
